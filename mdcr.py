...
class Mdcr(ControlSurface):
    def __init__(self, c_instance):
        ...
        with self.component_guard():
            ...
            self.mixer.channel_strip(0).set_arm_button(self._bcf._buttons[1][0])
            self.show_message("did it?")
            self.set_highlighting_session_component(self.session)
            self.init_clip_listeners()
            self.show_message(str(type(self.song().tracks)))
            self.log_message(str(type(self.song().tracks)))
    ...
    def handle_clip_launch(self, clip: Live.Clip.Clip):
        ...
        if clip.color_index in self._active_tracks:  # anything wrong with clip coloring?
            if self._active_tracks[clip.color_index] == track:  # same clip? return
                self.show_message("returing wrong")
                return
            else:
                ...
                with self.component_guard():
                    ...
                    #self.mixer.channel_strip(index).set_arm_button(self._bcf.get_arm_button(clip))
